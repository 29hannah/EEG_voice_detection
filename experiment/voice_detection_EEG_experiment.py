...
# labplatform https://github.com/sandriver03/slab-platform/blob/master/labplatform/core/ExperimentLogic.py
...
import logging
import random
from experiment.RX8RP2Device import RX8RP2Device
from experiment.config import vocoding_config

log = logging.getLogger(__name__)
cfg = voice_detection_EEG_config
...
class VoiceDetectionSetting(ExperimentSetting):
    ...
    trial_watch = Float(cfg.trial_duration, group='status', dsec='Duration of each stimulus, (s)')#see thread_func; trial duration  erhöhen, min Abstand sonst überlappt sich Aktivität
...
class VoiceDetectionExperiment(ExperimentLogic):
    ...
    sequence = slab.Trialsequence(conditions=setting.conditions,
                                  ...
                                  deviant_freq=setting.deviant_freq) #TODO integrate near equal transitions sequence here
    ...
    def load_stimuli(self): # TODO less complicated since only 5 sound files
        ...
        for ori_sound_file in os.listdir(os.path.join(get_config("SOUND_ROOT"), "neurobio2_2023\\vocoding\\original\\N_LUFS")):  # TODO syntax of dir? Why different to before?
            ...
            ori_sound.level = 70 # can be adjusted if necessary
